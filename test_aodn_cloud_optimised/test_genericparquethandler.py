...
TEST_FILE_NC = os.path.join(ROOT_DIR, 'resources', 'IMOS_ANMN-NSW_CDSTZ_20210429T015500Z_SYD140_FV01_SYD140-2104-SBE37SM-RS232-128_END-20210812T011500Z_C-20210827T074819Z.nc')
DUMMY_FILE = os.path.join(ROOT_DIR, 'resources',  'DUMMY.nan')
DUMMY_NC_FILE = os.path.join(ROOT_DIR, 'resources',  'DUMMY.nc')
TEST_CSV_FILE = os.path.join(ROOT_DIR, 'resources', 'A69-1105-135_107799906_130722039.csv')
...
class TestGenericHandler(unittest.TestCase):
    ...
    def setUp(self, mock_get_s3_raw_obj):
        ...
        self.tmp_dummy_file_path = os.path.join(self.tmp_dir, os.path.basename(DUMMY_FILE))
        shutil.copy(DUMMY_FILE, self.tmp_dummy_file_path)

        self.tmp_dummy_nc_path = os.path.join(self.tmp_dir, os.path.basename(DUMMY_NC_FILE))
        ...
        self.handler_csv_file.cloud_optimised_output_path = os.path.join(self.tmp_dir, "dummy_dataset_name")
    ...
    def test_add_columns_df(self):
        ...
        self.assertEqual(result_df['filename'].iloc[0], os.path.basename(self.tmp_nc_path))
    ...
    def test_create_data_parquet(self, mock_boto3_client):
        ...
        df['converted_polygon'] = df['polygon'].apply(lambda x: wkb.loads(bytes.fromhex(x)))
        ...
        self.assertNotIn('TEMP_quality_control', parquet_dataset.columns)  # make sure the variable is removed
        ...
        parquet_meta_file_path = os.path.join(self.handler.cloud_optimised_output_path, '_common_metadata')
        ...
        self.assertEqual(schema_dict['TEMP'].metadata.get(b'standard_name'), b'sea_water_temperature')
        # other way to access the metadata
        schema_dict = {obj.name: obj.metadata for obj in parquet_meta}
        self.assertEqual(schema_dict['TEMP'][b'standard_name'], b'sea_water_temperature')
