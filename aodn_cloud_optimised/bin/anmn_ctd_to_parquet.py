...
def main():
    ...
    # CTD
    ctd_ts_fv01_ls = [s for s in nc_obj_ls if ("CTD_timeseries" in s) and ("FV01" in s)]
    dataset_config = load_dataset_config(
        str(
            importlib.resources.path(
                "aodn_cloud_optimised.config.dataset", "anmn_ctd_ts_fv01.json"
            )
        )
    )

    cloud_optimised_creation_loop(ctd_ts_fv01_ls, dataset_config=dataset_config)
