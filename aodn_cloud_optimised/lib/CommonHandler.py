...
class CommonHandler:
    ...
    # def init_s3_filesystems(
    #     self,
    #     s3_bucket_opts: Optional[dict] = None,
    #     s3_fs_common_opts: Optional[dict] = None,
    #     s3_fs_common_session=None,
    # ):
    #     """
    #     Initialise S3FileSystem objects for input, output, and general access.
    #
    #     Args:
    #         s3_bucket_opts (Optional[dict]): Per-bucket S3 options (input/output).
    #         s3_fs_common_opts (Optional[dict]): Common S3 options to use if per-bucket not provided.
    #         s3_fs_common_session (optional): Optional session for S3FileSystem (used in testing with Moto).
    #
    #     Sets:
    #         self.s3_fs: General S3 filesystem (common or default)
    #         self.s3_fs_input: S3 filesystem for input_data bucket
    #         self.s3_fs_output: S3 filesystem for output_data bucket
    #         self.s3_client_opts: boto3 client options from input bucket (default)
    #     """
    #     # General fallback filesystem
    #     if s3_fs_common_opts:
    #         self.s3_fs = s3fs.S3FileSystem(**s3_fs_common_opts)
    #     else:
    #         self.s3_fs = s3fs.S3FileSystem(
    #             anon=False, default_cache_type=None, session=s3_fs_common_session
    #         )
    #
    #     # Input bucket
    #     if (
    ...
    #         self.s3_fs_output = self.s3_fs
    #
    #     # Default boto3 client options from input bucket
    #     if not getattr(self, "s3_client_opts", None):
    #         from aodn_cloud_optimised.lib.s3Tools import boto3_s3_from_opts_dict
    #
    #         if (
    #             s3_bucket_opts
    ...
    #             and s3_bucket_opts["input_data"].get("s3_fs_opts")
    #         ):
    #             self.s3_client_opts = boto3_s3_from_opts_dict(
    #                 s3_bucket_opts["input_data"]["s3_fs_opts"]
    #             )
    #         elif s3_fs_common_opts:
    #             self.s3_client_opts = boto3_s3_from_opts_dict(s3_fs_common_opts)
    #         else:
    #             self.s3_client_opts = None
