...
from aodn_cloud_optimised.lib.logging import ExitOnErrorLogger, get_logger
...
class TestCastTableTolerance(unittest.TestCase):
    ...
    def test_raises_named_valueerror_under_exit_on_error_logger(self):
        """The production logger must not mask which column failed.

        ``ExitOnErrorLogger.error`` raises a bare "Forcing script exit"
        Exception when raise_error is set, which would fire on the first bad
        column -- losing the column name and bypassing the ``except ValueError``
        at the call site. Failures are therefore logged at warning level in that
        mode, leaving the aggregated ValueError to propagate.
        """
        handler = _make_handler(raise_error=True)
        handler.logger = get_logger("test_aatams_exit_on_error", raise_error=True)
        self.assertIsInstance(handler.logger, ExitOnErrorLogger)
